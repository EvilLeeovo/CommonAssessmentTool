...
class ClientQueryService:
    ...
    def get_clients_by_case_worker(db: Session, case_worker_id: int):
        ...
        return db.query(Client).join(Client.cases).filter_by(user_id=case_worker_id).all()
    ...
    def get_clients_by_success_rate(db: Session, min_rate: int = 70):
        ...
        return db.query(Client).join(Client.cases).filter(Client.cases.success_rate >= min_rate).all()
