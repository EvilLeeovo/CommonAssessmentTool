...
def test_create_case_assignment_success(test_db):
    new_case = CaseAssignmentService.create_case_assignment(
        test_db, client_id=1, case_worker_id=2
    )
...
def test_update_client_services_success(test_db):
    ...
    updated = CaseAssignmentService.update_client_services(
        test_db, 1, 1, service_update
    )
